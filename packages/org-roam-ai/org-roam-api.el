...
(defun my/api--find-file-by-identifier (identifier)
  "Find org file by ID or relative path. Returns absolute path or nil."
  (let* ((by-id (ignore-errors
                  (org-roam-node-file
                   (org-roam-node-from-id identifier))))
         (by-path (when (not by-id)
                    (let ((full (expand-file-name identifier org-roam-directory)))
                      (when (file-exists-p full) full)))))
    (or by-id by-path)))

(defun my/api-read-note (identifier &optional section)
  "Read content of a note by ID or relative path.
If SECTION is provided, return only that heading's content."
  (let ((file (my/api--find-file-by-identifier identifier)))
    (if (not file)
        (my/api--json-response `((success . nil) (error . ,(format "Note not found: %s" identifier))))
      (with-current-buffer (find-file-noselect file)
        (if (not section)
          ...
          (if (re-search-forward (format "^\\*+ +%s" (regexp-quote section)) nil t)
            ...
            (my/api--json-response
             `((success . nil)
               (error . ,(format "Section not found: %s" section))))))))))

(defun my/api-update-note (identifier content &optional section mode)
  "Update a note by ID or relative path.
MODE is append (default), prepend, or replace.
If SECTION is provided, target that heading."
  (let ((file (my/api--find-file-by-identifier identifier))
        (mode (or mode "append")))
    (if (not file)
        (my/api--json-response `((success . nil) (error . ,(format "Note not found: %s" identifier))))
      (with-current-buffer (find-file-noselect file)
        (if section
            ...
            (progn
              (goto-char (point-min))
              (if (re-search-forward (format "^\\*+ +%s" (regexp-quote section)) nil t)
                  (let ((heading-end (progn (end-of-line) (point)))
                        (section-end (progn
                                       (outline-next-heading)
                                       (if (eobp) (point-max) (point-at-bol)))))
                    (cond
